...
If any changed files are under `clem2itunes/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `clem2itunes/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `clem2itunes/commands/main.py`,
...
For Python files, strip the `clem2itunes/` prefix and replace `/` with `.` (like module imports).

- Python file `clem2itunes/media.py` → `media:`.
- Multiple files under `clem2itunes/commands/` → `commands:`.
- Single command file `clem2itunes/commands/admin.py` → `commands.admin:`.
