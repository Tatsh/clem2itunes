...
Fixes mypy type errors and eliminates `Any` usage in the clem2itunes project.
...
1. Fix all mypy errors reported by `uv run mypy clem2itunes`
...
Place new TypedDict classes in `clem2itunes/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`clem2itunes/typing.py` when shared.
...
1. Run `uv run mypy clem2itunes` and capture all errors.
...
4. Run `uv run mypy clem2itunes` again to verify fixes.
